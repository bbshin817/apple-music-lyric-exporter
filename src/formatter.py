...
class Lyrics:
	...
	def addLyric(self, words, begin, end, syllables):
		self.lyrics.append({
	...
	for div in xml[1]:
		for p in div:
			if (p.text):
				...
				lyrics.addLyric(syllables.words, p.get('begin'), p.get('end'), syllables.syllables)
		if (divIndex < len(xml[1]) - 1):
			lyrics.addLyric('', div.get('end'), xml[1][divIndex + 1].get('begin'), [])
		else:
			lyrics.addLyric('', div.get('end'), xml[1].get('dur'), [])
		divIndex += 1
